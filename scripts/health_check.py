...
class HealthChecker:
    ...
    def save_results(self, results: Dict[str, Any]) -> None:
        ...
        historical_file = output_dir / f"health_check_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        with open(historical_file, 'w') as f:
            json.dump(results, f, indent=2)
    
    def run_continuous(self) -> None:
        """Run health checks continuously."""
        logger.info(f"Starting continuous health checks every {self.check_interval} seconds")
        while True:
            try:
