...
def plot_defect_trend(history_data: List[Dict[str, Any]]) -> go.Figure:
    ...
    daily_stats = df.groupby('date').agg({
        'defect_detected': ['count', 'sum']
    }).reset_index()
    
    daily_stats.columns = ['date', 'total_predictions', 'defects_found']
    daily_stats['defect_rate'] = (daily_stats['defects_found'] / daily_stats['total_predictions']) * 100
...
def plot_performance_metrics(history_data: List[Dict[str, Any]]) -> go.Figure:
    ...
    hourly_stats = df.groupby('hour').agg({
        'processing_time': ['mean', 'max', 'count']
    }).reset_index()
...
def create_monitoring_dashboard(history_data: List[Dict[str, Any]]) -> Dict[str, go.Figure]:
    ...
    return {
        ...
        'roc': plot_roc_curve(
            df['defect_detected'].astype(int).values,
            df['defect_probability'].values
        )
    }
