...
def create_dataset(
    ...
    no_defect_images = list(no_defect_dir.glob("*.jpg")) + list(
        no_defect_dir.glob("*.jpeg")
    )
    ...
    # Create dataset
    dataset = tf.data.Dataset.from_tensor_slices(([str(x) for x in all_images], labels))
    ...
    dataset = dataset.map(
        lambda x, y: (preprocess_image(x), y), num_parallel_calls=tf.data.AUTOTUNE
    )
